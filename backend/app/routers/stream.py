...
def process_audio_chunk(
    ...
    try:
        ...
        return {
            ...
            "elapsed_seconds": round(elapsed_seconds, 2) if elapsed_seconds is not None else 0.0,
            "actual_chunk_duration": round(actual_chunk_duration_sec, 2) if actual_chunk_duration_sec is not None else 0.0,
            "actual_chunk_samples": actual_chunk_samples,
            "total_audio_duration": round(total_audio_duration_sec, 2) if total_audio_duration_sec is not None else None,
            "is_padded": is_padded,
            "nominal_window_sec": round(chunk_index * settings.AUDIO.CHUNK_DURATION_SEC, 2) if chunk_index is not None else 0.0,
            "chunk_risk_score": chunk_risk,
            "rolling_risk_score": rolling_risk,
            "status_color": status_color,
            "severity": severity,
